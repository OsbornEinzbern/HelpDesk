<template>
  ...
  <div class="all-objects-page">
    ...
    <SuccessNotification ref="successProcessModal" />
    <!-- Панель с кнопками над таблицей -->
    <div class="table-controls">
      <div class="table-controls-left">
        <button 
          v-if="canDeleteProcess"
          type="button" 
          class="btn danger" 
          @click.stop="handleSearch" 
          :disabled="!selectedProcessIds || selectedProcessIds.length === 0"
        ...
        <div class="search-section">
          <UIInput
            v-model="searchQuery"
            placeholder="Поиск по бизнес-процессам..."
            type="text"
            :max-length="30"
          ...
          <button class="search-btn" @click="handleSearch">
            <Icon :icon="uiIcons?.icons.search" width="24" height="24" />
          </button>
        </div>
      </div>
      ...
      <div class="table-controls-right">
        ...
        >
          <span class="btn-content">
            <Icon :icon="uiIcons?.icons.add" width="22" height="22" />
            <span>Создать бизнес-процесс</span>
          </span>
        </UIButton>
      </div>
    </div>
    ...
    <div class="table-section">
      <SupportGroupsTable
        ...
        :selectedGroupIds="selectedProcessIds || []"
    ...
    <!-- Модалка профиля -->
    <BusinessProcessProfileModal
      v-model="modalVisible"
      :mode="modalMode"
      :process="selectedProcess"
      @saved="handleProcessSaved"
      @deleted="handleProcessDeleted"
    />
  </div>
</template>
...
import BusinessProcessProfileModal from '@/components/modal/BusinessProcessProfileModal.vue'
...
const getDeleteButtonTitle = computed(() => {
  if (!selectedProcessIds.value || selectedProcessIds.value.length === 0) {
    return 'Выберите бизнес-процессы для удаления'
  }
  return `Удалить выбранные бизнес-процессы поддержки (${selectedProcessIds.value.length})`
})
    ...
    if (response && response.data) {
      if (response.data.data) {
        ...
        paginationData.value = {
          current_page: 1,
          last_page: 1,
          per_page: groups.value.length || 20,
          total: groups.value.length || 0,
          links: [],
          from: 1,
          to: groups.value.length || 0,
        }
      } else {
        groups.value = []
...
const deleteSelectedProcesses = async () => {
  if (!selectedProcessIds.value || selectedProcessIds.value.length === 0) return
  ...
  const confirmed = await deleteNotification.value?.open({
    ...
    secondaryMessage: `Выбрано групп: ${selectedProcessIds.value.length}. Это действие необратимо.`,
    confirmText: 'Удалить'
  })
  ...
  try {
    const payload = { support_groups: selectedProcessIds.value }
    const result = await supportGroupsStore.deleteGroups(payload)
    
    if (!result.validator_fails) {
      const confirmed2 = await successProcessModal.value.open({
        title: '',
      ...
      await successProcessModal.value.open({
        title: '',
